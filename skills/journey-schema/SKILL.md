...
- Directly via `/butterbase:journey-schema`.
...
4. **Hackathon mode only:** if `00-state.md` has `hackathon_mode: true`, also invoke `butterbase:journey-rls` inline now. After it returns, mark the rls checkbox as `(folded)` in `00-state.md` rather than leaving it unchecked.
